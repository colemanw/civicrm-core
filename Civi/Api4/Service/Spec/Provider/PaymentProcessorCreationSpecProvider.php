...
class PaymentProcessorCreationSpecProvider extends \Civi\Core\Service\AutoService implements Generic\SpecProviderInterface {
  ...
  public function modifySpec(RequestSpec $spec) {
    // Name is autogenerated from title, if present.
    $spec->getFieldByName('name')->setRequired(FALSE);
    $spec->getFieldByName('name')->setRequiredIf('empty($values.title)');
    // Title is supplied from name if missing
    $spec->getFieldByName('title')->setRequired(FALSE);
    $spec->getFieldByName('title')->setRequiredIf('empty($values.name)');
    // Frontend_title is supplied from title if missing
